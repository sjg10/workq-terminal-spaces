 ...
 *
 * We also implement an automatic check for the queue to be
 * dumped to disk if memory runs low, and to recall dumps 
    ...
    void*         (*load_element)(FILE *file))          /*load routine*/
{
    ...
    wq->queue_length=0;
    wq->file_counter=0;
    wq->save_element=save_element;
    wq->load_element=load_element;
    wq->engine = engine;
...
int workq_add (workq_t *wq, void *element)
{
     ...
     * first and last pointers and queue length.
     */
    if (wq->first == NULL)
        wq->first = item;
    else
...
/*Dumps the current contents of the workqueue to disk*/
int workq_dump(workq_t *wq){
    DPRINTF (("DUMPING TO FILE\n"));
    //only to be called within mutexted enviroment.
    workq_ele_t *we,*temp;
    int i,j;
    FILE *file;
    int status;
    ...
    for(i=0;i<3;i++){
        //prepare filename
        char filename[30]="workqueuedump";
        wq->file_counter++;
        char str[15];
        sprintf(str, "%d", wq->file_counter);
        strcat(filename, str);
        file=fopen(filename,"wb");
        if(file == NULL) {
          return EBADF;
        }
        for(j=0;j<MAX_Q_LENGTH/4;j++) {
            wq->save_element(we->data,file);
            wq->first = we->next;
            free(we);
            we=wq->first;
            wq->queue_length--;
...
/*Loads last disk sump to work queue*/
int workq_load(workq_t *wq){
    DPRINTF (("LOADING FROM FILE\n"));
    //only to be called within mutexted enviroment.
    workq_ele_t *we,*temp;
    void *element;
    int c,i;
...
/*Checks if we need to dump to or pull from disk to clear/fill qork queue*/
int workq_load_unload(workq_t *wq){
    int status;
    int worked=0;
    status = pthread_mutex_lock (&wq->mutex);
    if (status != 0) {
    ...
    if (wq->queue_length > MAX_Q_LENGTH){
        workq_dump(wq);
        worked=1;}
    else if(wq->queue_length<=NO_OF_THREADS && wq->file_counter>0){
        workq_load(wq);
        }
    status = pthread_mutex_unlock (&wq->mutex);
    if (status != 0)
